...
def test_wait_returns_on_completed(
    ...
    with respx.mock(base_url="https://sepal.test") as mock:
        mock.get("/api/tasks/task/t-1").mock(
            side_effect=lambda req: httpx.Response(
                200, json={"id": "t-1", "state": next(states)}
            )
        )
...
def test_wait_failed_raises(
    http: httpx.Client, monkeypatch: pytest.MonkeyPatch
) -> None:
    with respx.mock(base_url="https://sepal.test") as mock:
        mock.get("/api/tasks/task/t-1").respond(
            200, json={"id": "t-1", "state": "FAILED"}
        )
        monkeypatch.setattr("time.sleep", lambda _: None)
        endpoint = TasksEndpoint(http)
...
def test_wait_canceled_raises(
    http: httpx.Client, monkeypatch: pytest.MonkeyPatch
) -> None:
    with respx.mock(base_url="https://sepal.test") as mock:
        mock.get("/api/tasks/task/t-1").respond(
            200, json={"id": "t-1", "state": "CANCELED"}
        )
        monkeypatch.setattr("time.sleep", lambda _: None)
        endpoint = TasksEndpoint(http)
